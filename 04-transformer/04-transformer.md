...
![Transformer 整体架构](../assets/images/ch18-transformer-arch.png)

> 🎨 生图提示词：Transformer 架构图（论文风格但更友好）：左侧 Encoder 堆叠（词嵌入+位置编码→多头注意力→前馈网络，×N），右侧 Decoder 堆叠（含 Masked 注意力与 Cross-Attention 指向 Encoder），组件用彩色矩形框+中文标注，箭头示意信息流，蓝色 Encoder/红色 Decoder/绿色 Attention，简洁教学风。
...
![位置编码：给每个词发一个座位号](../assets/images/ch18-positional-encoding.png)

> 🎨 生图提示词：位置编码示意图，教学插画风格，中文标注。一排词块"我 / 爱 / 北 / 京"，每个词块下方叠加一张"座位号票"（1、2、3、4），用"+"号表示词向量与位置编码相加。旁边小字提示"狗咬人 ≠ 人咬狗"。配色友好，突出"词向量 + 位置信息"的叠加概念。
...
![多头注意力：多个头从不同角度理解同一句话](../assets/images/ch18-multi-head-attention.png)

> 🎨 生图提示词：多头注意力示意图，教学插画风格，中文标注。中间一句话"小明把书借给了小红"，从这句话分出 8 条彩色通道（8 个"头"），每条通道旁标注不同关注点（语法、指代、情感、时序……），每条通道内画出不同的词间连线，最后 8 条通道汇聚到一个"汇总"方块。用不同颜色区分不同的头，风格清爽教学向。

我们还可以把每个"头"关注的重点，画成一张**注意力热力图**——颜色越深，代表两个词之间的关注度越高，非常直观。

![注意力热力图：颜色越深关注度越高](../assets/images/ch18-attention-heatmap.png)

> 🎨 生图提示词：注意力热力图（heatmap），教学风格，中文标注。一个方形网格，横轴和纵轴都是同一句话的词"小明 / 把 / 书 / 借给 / 小红 / 他"，格子用蓝到深红的渐变色表示关注度高低，"他"与"小明"交叉的格子颜色最深。配色专业但清晰，附上色阶图例"关注度低→高"。
